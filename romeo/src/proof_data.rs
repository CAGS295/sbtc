...
use blockstack_lib::{
	util::hash::{DoubleSha256, MerklePath, MerkleTree},
	vm::types::{ListData, ListTypeData, SequenceData, Value, BUFF_32},
};
...
impl ProofData {
	...
	pub fn from_block_and_index(block: &Block, index: usize) -> Self {
		...
		let txid_vecs =
			block.txdata.iter().map(|tx| tx.txid().to_vec()).collect();
	...
	pub fn to_values(&self) -> ProofDataClarityValues {
		...
		ProofDataClarityValues {
			txid: Value::buff_from(self.txid.to_vec())
			...
			merkle_path: Value::Sequence(SequenceData::List(ListData {
				data: self
					...
					.iter()
					.map(|v| {
						Value::buff_from(v.hash.to_bytes().to_vec()).unwrap()
					})
					.collect(),
